...
const Extension = ({ context, runServerless, sendAlert, actions, openIframe }) => {
  ...
  const [appointmentSubTypeOptionsFiltered, setAppointmentSubTypeOptionsFiltered] = useState([]);
    ...
    runServerless({ name: 'getHubDB' }).then(
      (serverlessResponse) => {
        if (serverlessResponse.status == 'SUCCESS') {
          ...
          console.log("meetingSubTypeTable");
          console.log(meetingSubTypeTable);

          let meetingSubTypeOptions = meetingSubTypeTable.columns;
          setAppointmentSubTypeOptions(meetingSubTypeOptions);
          console.log("meetingSubTypeOptions");
          console.log(meetingSubTypeOptions);
        }
      }
    );
          ...
          <Flex direction={'row'} gap={'extra-large'} alignSelf={'start'}>
            <Box flex={1} alignSelf="end">
              <Select
                ...
                onChange={(e) => {
                  ...
                  if (!e) {
                    let filteredTypes = appointmentSubTypeOptions.options.filter(obj => obj.meetingTypeCode == e);
                    console.log(filteredTypes);
                    filteredTypes.map(obj => (obj.label = obj.name, obj.value = obj.name));
                    setAppointmentSubTypeOptionsFiltered(filteredTypes);
                  }
                  else {
                    setAppointmentSubTypeOptionsFiltered([]);
                  }
                }}
                variant="primary"
                buttonSize="md"
            ...
            <Box flex={1} alignSelf="end">
              <Select
                ...
                options={appointmentSubTypeOptionsFiltered}
